...
int[,,] array = GetArray(new int[] {int.Parse(nums[0]), int.Parse(nums[1]), int.Parse(nums[2]) }, minElements,maxElements);
PrintArray(array);
...
int[,,] GetArray(int[] sizes, int min, int max)
{
    ...
    for (int i = 0; i < result.GetLength(0); i++)
    {
        for (int j = 0; j < result.GetLength(1); j++)
        {
            ...
            {
               int element = new Random().Next(minElements, maxElements+1);
